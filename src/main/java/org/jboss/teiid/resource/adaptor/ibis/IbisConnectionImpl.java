...
public class IbisConnectionImpl extends BasicConnection {
	...
	public IbisConnectionImpl(
			IbisManagedConnectionFactory mcf) {
		this.mcf = mcf;
	}
	...
	private String request(String url, HttpMethod httpMethod, int timeout) throws HttpException, ClientProtocolException, IOException
             {
    ...
    default:
            throw new HttpException("Unexpected HTTP method {0} " + httpMethod.toString());
    }
   
   
}
	private String getResponse(String url, int timeout) throws ClientProtocolException, IOException {
	...
	public Object executeQuery(final String params) throws java.lang.Exception {
	    
		url = getURL(this.mcf.getEndPoint(), params);
		String json = request(url, HttpMethod.GET , timeout);
...
//	    final Messages messages = createMessages(rootNode.get(IOpenShiftJsonConstants.PROPERTY_MESSAGES));
    ...
    private ModelNode getModelNode(final String content) throws  java.lang.Exception {
        if (content == null) {
                throw new java.lang.Exception("Could not unmarshall response: no content.");
        }
        final ModelNode node = ModelNode.fromJSONString(content);
        if (!node.isDefined()) {
                throw new  java.lang.Exception("Could not unmarshall response: erroneous content.");
        }
		...
		if (endpoint != null) {
			try {
				...
				if (defaultEndpoint == null) {
					throw new WebServiceException("null_default_endpoint"); //$NON-NLS-1$
				}
			...
			if (endpoint == null) {
				throw new WebServiceException(("null_endpoint")); //$NON-NLS-1$
			}
		}
